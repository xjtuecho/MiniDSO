...
#define TITLE_L   3
#define RowHeight 14
#define MAX_FREQ  10                             /*输出模拟波形最大频率为20KHz*/
...
//--------------+------+------+------+------+------+------+------+------+------+
uc8  BT_S[][5] ={ ";<=", ";<=", ";&@", ";*@", ">`@", "\"#$", "\"#$"};// 符号串 |
uc16 V_BT[]    ={  3000,  3400,  3700,  3850,  4200,  4200,  4200};  // 电压值 |
...
//------------FILE manage-----------------------------------------------------// 
uc8 FileStr[][20] = {
 "Save File  .Pam","Save File  .BMP","Save File  .DAT",
 "Save File  .BUF","Save File  .CSV","Load File  .DAT","Load File  .BUF",};
...
uc8 Item_Str[][10]= {
  "  CH_A   ",  "  CH_B   ",  "  CH_C   ", 
  "TimeBase ",  "Trigger  ",  "Cursors   ",  "Window    ",
  "         ",  "         ",  "         ",   "         ",  
  "         ",   "          ",
...
uc8  Math_Str[][6]      = {" -A ", " -B ", "A+B ", "A-B ",
                           "RecA", "RecB", "RecC"};
uc8  Swintch[][7]       ={" ON  "," OFF  "};
uc8  Swintch1[][7]      ={"ON   ","OFF   "};
uc8  Swintch_V[][7]     ={" CH_A"," CH_B"," OFF "};
uc8  T0_Str[][7]        ={"  1  ","  2  ","  3  "};
uc8  Adc_Str[][7]       ={" AC  "," DC  "};
...
uc16 Title_Posi_X[] = { 6,   29,   83, 140, 180, 240, 289,  };
...
uc8  PopMenu1_Limit[] = { 4,   4,   3,    1,    6,    6,   3};//关闭T0 Fit
//---Popmenu_3选项数目---FILE| WAVE| SYS | CAL | PRO| ABOUT
uc8  PopMenu3_Base[]  = { 0,    7,   10,    17,   19,   21}; 
uc8  PopMenu3_Limit[] = { 7,    3,    7,     2,    2,    2};
...
//======+====+=====+======+=====+=====+=======+   
//CH1    VOL | POST| AC/DC| FIT | ENABLE
          9,   195,    1,    1,     1,   
//CH2    VOL | POST| AC/DC| FIT | ENABLE     
          9,   195,    1,    1,     1,
...
sc16 PopMenu1_Limit2[]={ 
  1,  5,  0,   0,  0,           //CH1     VOL | POST| AC/DC| FIT | ENABLE
  1,  5,  0,   0,  0,           //CH2     VOL | POST| AC/DC| FIT | ENABLE
...
//--------------Popmenu选项默认值------------------------
s16 PopMenu1_Value[]={
  7,  100,  0,  0,  1,        //CH1     VOL | POST| AC/DC| FIT | ENABLE
  7,   85,  0,  0,  1,        //CH2     VOL | POST| AC/DC| FIT | ENABLE
  2,   30,  1,                //CH3    TYPE | POTS| SWITCH
  7,    1,                    //Time   TIME | FIT
  0,    0,  0, 10,   0,  1, 8,//Trig   MODE | TYPE| CH_N | THRI| FIT |ENABLE|THR2
  50, 200,  0,175,  25,  0,   //Curs    T1  |  T2 | EN_T |  V1 |  V2 |EN_V 
  0,    2,  0,  0,            //Win    POSI |DEPTH| EN           
};
...
uc16 PopMenu3_Limit1[]={//上限 
  99, 99, 99, 99, 99, 99, 99,    //File   Pam | BMP | DAT | BUF | CSV | DAT |BUF 
  3, 15,  9,                     //Wave   TYPE| FRQ | DUTY| 
  9,  10, 30, 30, 1, 1, 1,       //Sys    VOL | BK  | STANDBY   | PowerOff
};
...
void Show_Menu(void)
{
  ...
  if(menu.mflag & UPD){
    ...
    Draw_Circle_D(RED,  Menu_X, Menu_Y, 38, 53, 56);  //画菜单标题页数矩形框
  ...
  if(menu.current==Measure){
    ...
    for(n=0; n<Menu_Limit[menu.current]; n++){
      ...
      Show_Item(k);                         //max=16  item+n为Pop的第k项
    }
  }
}
...
void Show_Item(u8 k)
{
  ...
  u16 x,y,j;
    switch (k){
    case CH1:
      wink = (PopMenu1_Value[CH1_Fit])? PRN : VOID; //F模式时，字体半消隐
      Sy=ITEM_Y-rowh+3;
      PrintStr6x8(wink, (u8*)VolMenu_Str[PopMenu1_Value[CH1_Vol]]);
      Sy=ITEM_Y-rowh;
      Draw_Circle_S(CH_Col[k], Menu_X, Sy, 28, 6);  //画项目栏颜色
      wink = (PopMenu1_Value[CH1_Fit])? INV : INV;  //F模式时，字体半消隐
      Draw_Circle_D(CH_Col[k], Title_Posi_X[k+1]+8, TITLE_H+2, 11, 2, 7*6);
      SetColor(DAR,CH_Col[k]);
      DispStr(Title_Posi_X[k+1]-1, TITLE_H+2, SYMB,
             (PopMenu1_Value[CH1_Fit]?(u8*)("_"):(u8*)("i")));
      DispStr6x8(Title_Posi_X[k+1]+11,TITLE_H+2, wink,(PopMenu1_Value[CH1_Enable]?
                (u8*)("Hide "):(u8*)(Vol_Str[PopMenu1_Value[CH1_Vol]]+1)));
      DispStr(Title_Posi_X[k+1]+40, TITLE_H+2, wink+SYMB,
             (u8*)(Cps_SYMB[PopMenu1_Value[CH1_Coupl]]));
      break;  
    case CH2:
      wink = (PopMenu1_Value[CH2_Fit])? PRN : VOID; //F模式时，字体半消隐
      Sy=ITEM_Y-rowh+3;
      PrintStr6x8(wink, (u8*)VolMenu_Str[PopMenu1_Value[CH2_Vol]]);
      Sy=ITEM_Y-rowh;
      Draw_Circle_S(CH_Col[k], Menu_X, Sy, 28, 6);  //画项目栏颜色
      SetColor(DAR,CH_Col[k]);
      wink = (PopMenu1_Value[CH2_Fit])? INV : INV;  //F模式时，字体半消隐
      Draw_Circle_D(CH_Col[k], Title_Posi_X[k+1]+8, TITLE_H+2, 11, 2, 7*6);
      DispStr(Title_Posi_X[k+1]-1, TITLE_H+2, SYMB,
             (PopMenu1_Value[CH2_Fit]?(u8*)("_"):(u8*)("i")));
      DispStr6x8(Title_Posi_X[k+1]+ 7+4,TITLE_H+2 ,wink,(PopMenu1_Value[CH2_Enable]?
                (u8*)("Hide "):(u8*)(Vol_Str[PopMenu1_Value[CH2_Vol]]+1)));
      DispStr(Title_Posi_X[k+1]+40, TITLE_H+2, wink+SYMB,
    ...
    case CH3: 
      ...
      DispStr6x8(Title_Posi_X[k+1]+12,TITLE_H+2,INV,(PopMenu1_Value[CH3_Enable]?
                (u8*)("Hide"):(u8*)(Math_Str[PopMenu1_Value[CH3_Type]])));
      break;
    case TIMEBASE:        
    ...
    case TRIGGER:
      ...
      DispStr(Title_Posi_X[k+1]-12, TITLE_H+2, SYMB,
             (PopMenu1_Value[TRI_Fit]?(u8*)("_"):(u8*)("i")));
      Update_Status();
      break;
    case COURSOR:
      Draw_Circle_S(CH_Col[k], Menu_X, Sy, 14, 6);//画项目栏颜色
 ...
 //-------------------第三页------------------------------------------
    case FILE:
      ...
      Sy=ITEM_Y-rowh;
      Draw_Circle_S(CH_Col[0], Menu_X, Sy, 28, 6);//画项目栏颜色
      break;
    case WAVE:
      Sx=Menu_X+8;                                //菜单标题
      Sy=ITEM_Y-rowh+3; 
      PrintStr6x8(PRN, (u8*)&Options_Str[k-FILE]);
      Sy=ITEM_Y-rowh;
      Draw_Circle_S(CH_Col[1], Menu_X, Sy, 28, 6);//画项目栏颜色
      break;
    case SYSTEM:
      Sx=Menu_X+8;                                //菜单标题
      Sy=ITEM_Y-rowh+3;      
      PrintStr6x8(PRN, (u8*)&Options_Str[k-FILE]);
      Sy=ITEM_Y-rowh;
      Draw_Circle_S(CH_Col[2], Menu_X, Sy, 28, 6);//画项目栏颜色
      break;
    case CALIBRATION:
      Sx=Menu_X+8;                                //菜单标题
      Sy=ITEM_Y-rowh+3;      
      PrintStr6x8(PRN, (u8*)&Options_Str[k-FILE]);
      Sy=ITEM_Y-rowh;
      Draw_Circle_S(ORN, Menu_X, Sy, 28, 6);      //画项目栏颜色
      break;
    case PRO:
      Sy=ITEM_Y-rowh+3;
      PrintStr6x8(PRN, (u8*)&Options_Str[k-FILE]);
      Sy=ITEM_Y-rowh;
      Draw_Circle_S(BLU, Menu_X, Sy, 28, 6);      //画项目栏颜色
      break;
    case ABOUT:
      Draw_Circle_S(RED, Menu_X, Sy, 14, 6);      //画项目栏颜色
      break;
    }
    rowh++;
    x=Menu_X+2;
...
void Show_PopMenu(u8 Cur_Item)
{
  ...
  case Oscillo:
    ...
    case CH1:
      memcpy(&ptr," ",1);
      ptr2=(u8*)&Vol_Str[PopMenu1_Value[CH1_Vol]];                //Ch_range
      ...
      memcpy(&ptr[4*LEN],&Swintch[PopMenu1_Value[CH1_Fit]],5);    //Ch_Fit
      break;
    case CH2:
      memcpy(&ptr," ",1);
      ptr2=(u8*)&Vol_Str[PopMenu1_Value[CH2_Vol]];                //Ch_range
      memcpy(&ptr[1],&ptr2[1],LEN-5);
      memcpy(&ptr[LEN]," ",1);
      ...
      memcpy(&ptr[4*LEN],&Swintch[PopMenu1_Value[CH2_Fit]],5);    //Ch_Fit
      break;
    case CH3:
  ...
  case Option:
    ...
    case SYSTEM:
      ...
      memcpy(&ptr[LEN*6],":",1);
      memcpy(&ptr[LEN*6+1],(u8*)&Swintch1[PopMenu3_Value[SYS_PosiSlide]],5);
...
void Show_Measure(void)
{
  ...
  for(k=0; k<6; k++){
    ...
    if(menu.menu_index[menu.current]==k)Background=GRN;
      ...
      if(enable==0){ Background=WHT;  Foreground=BLK;}
      else          {Background=GRY; }
    }
    Sx=Menu_X+6;
    rowh+=RowHeight;
...
//------------------------------第一页菜单------------------------------------//
  case Oscillo:
    ...
    case CH1:  
      if((Status != STOP)){
      StateA = PopMenu1_Value[CH1_Enable]? GND : ACT ;  //Ch1接地或激活状态 
      GainA  = PopMenu1_Value[CH1_Vol]%5;               //？？？？
      KindA  = PopMenu1_Value[CH1_Vol]/5;               //Ch1选择低压或高压量程
      CouplA = ((PopMenu1_Value[CH1_Coupl])? DC : AC);  //AC/DC耦合方式
      __Ctrl(AiRANGE, KindA+CouplA+StateA);             //Ch1状态刷新
      }
      ParamTab[P1x2]=2*(PopMenu1_Value[CH1_Posi]);      //Ch1游标位置 
      ParamTab[W1F]=PopMenu1_Value[CH1_Enable]? B_HID : L_HID;  //Ch1 show_hide 
      ...
      ParamTab[VTF]=(PopMenu1_Value[TRI_Enable])? L_HID : W_HID;
      if(Cur_PopItem==2)Update[W1F]|=UPD;               //游标刷新标志
      break;
    case CH2:
      if((Status != STOP)){
      StateB = PopMenu1_Value[CH2_Enable]? GND : ACT ;  //Ch2接地或激活状态  
      GainB  = PopMenu1_Value[CH2_Vol]%5; 
      KindB  = PopMenu1_Value[CH2_Vol]/5;
      CouplB = ((PopMenu1_Value[CH2_Coupl])? DC : AC);
      __Ctrl(BiRANGE, KindB+CouplB+StateB);
      }
      ParamTab[P2x2]=2*(PopMenu1_Value[CH2_Posi]);
      ParamTab[W2F]=PopMenu1_Value[CH2_Enable]? B_HID : L_HID;
      ...
      break;
    case CH3:
      ParamTab[W3F]=PopMenu1_Value[CH3_Enable]? B_HID : L_HID;
    ...
    case COURSOR:
      ...
      if(PopMenu1_Value[CUR_V]==2)                     // CH1,Ch2,OFF
        ParamTab[V1F]=L_HID;
...
void FMenu_Pop(u8 item,u8 current,u8* str1)
{
  ...
  PopColor(WHT, DAR); 
  PopType |= FILE_POP;
  OpenPop(PopMenu3_Posi_X[j], PopMenu1_Posi_Y[j],(item)*16+4,  14*6,  COVER); 
  ptr1=str1;
  Wink = INV;
  for(i = 1; i <= item; i++){
    Px = 6;
...
void MeasureStr(u8 Source, u8 Item)
{
  ...
  case 2: // Vrms 
    if(Source == CH1){
      scale = VScale[PopMenu1_Value[CH1_Vol]]*40;
      Temp  = ((((PrmsA)*Ak)>>12))*scale;
    }else{
      scale = VScale[PopMenu1_Value[CH2_Vol]]*40;
  ...
  case 3: // Vavg
    if(Source == CH1){
      scale = VScale[PopMenu1_Value[CH1_Vol]]*40;
      Temp  = ((((PavgA-2048)*((KgA[KindA]*4)/GK[GainA+5*KindA]))>>12)
               +100-PopMenu1_Value[CH1_Posi])*scale;
    }else{
      scale = VScale[PopMenu1_Value[CH2_Vol]]*40;
      Temp  = ((((PavgB-2048)*((KgB[KindB]*4)/GK[GainB+5*KindB]))>>12)
               +100-PopMenu1_Value[CH2_Posi])*scale;
    }
    if(fabs(Temp)<(scale*25*10/100)){
      Temp=0;
      Value2Str(NumStr, Temp, (u8*)NSTR, 3, SIGN);} 
  ...
  case 4: // Vp-p
    if(Source == CH1){
      scale = VScale[PopMenu1_Value[CH1_Vol]]*40;
      Temp  = ((((PmaxA-2048)*((KgA[KindA]*4)/GK[GainA+5*KindA]))>>12)
               -(((PminA-2048)*((KgA[KindA]*4)/GK[GainA+5*KindA]))>>12))*scale;
    }else{
      scale = VScale[PopMenu1_Value[CH2_Vol]]*40;
      Temp  = ((((PmaxB-2048)*((KgB[KindB]*4)/GK[GainB+5*KindB]))>>12)
               -(((PminB-2048)*((KgB[KindB]*4)/GK[GainB+5*KindB]))>>12))*scale;
    }
  ...
  case 6: // Vmin
    if(Source == CH1){
      scale = VScale[PopMenu1_Value[CH1_Vol]]*40;
      Temp  = ((((PminA-2048)*((KgA[KindA]*4)/GK[GainA+5*KindA]))>>12)
               +100-PopMenu1_Value[CH1_Posi])*scale;
    }else{
      scale = VScale[PopMenu1_Value[CH2_Vol]]*40;
      Temp  = ((((PminB-2048)*((KgB[KindB]*4)/GK[GainB+5*KindB]))>>12)
               +100-PopMenu1_Value[CH2_Posi])*scale;
    }
    if(fabs(Temp)<(scale*25*10/100)){
      Temp=0;
...
void Print_dV_Info(u8 Wink)
{
  s32  Tmp,scale;
  u8 index;
  index=(PopMenu1_Value[CUR_V])?PopMenu1_Value[CH2_Vol]:PopMenu1_Value[CH1_Vol];
  scale=VScale[index];
  Tmp =scale *((ParamTab[V1x2]-ParamTab[V2x2])/2)/25;
  Value2Str(NumStr, Tmp, (u8*)&YSTR[1], 3, SIGN);
  Sx=0,Sy=0;
  SetColor(Background, CH_Col[PopMenu1_Value[CUR_V]]); 
  Draw_Circle_D(CH_Col[PopMenu1_Value[CUR_V]], 1*6-3, TITLE_L, 11, 2, 11*6-2);
  DispStr6x8(6,   TITLE_L, INV, "          ");
