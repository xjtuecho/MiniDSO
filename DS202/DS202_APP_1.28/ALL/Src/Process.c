/******************** (C) COPYRIGHT 2014 e-Design Co.,Ltd. *********************
 File Name : Process.c
 Version   : DS202 APP Ver 1.00                                    Author : bure
*******************************************************************************/
#include "Version.h"
#include "Process.h"
#include "Bios.h"
...
//-------+------+------+------+------+------+------+------+------+------+------+
uc8 GK[]={    1,     2,     5,    10,    20,     1,     2,     4,    10,    20,}; 
//-------+------+------+------+------+------+------+------+------+------+------+
...
u32  HighA, HighB, LowA,  LowB,  EdgeA, EdgeB;
 ...
 //-------------+-------+-------+-------+-------+-------+-------+-------+-------+
s16   Kpg[] =    {  2044,   2046,   2011,   2046,   1240,   1240,   1240,   1240,}; 
sc16 Kpgc[] =    {  2005,   2053,   1983,   2058,   1254,   1261,   1254,   1256,};
//=============+=======+=======+=======+=======+=======+=======+=======+=======+
s16 *KpA = &Kpg[0], *KpB = &Kpg[2], *KgA = &Kpg[4], *KgB = &Kpg[6];
...
{ 
  ...
  for(i=0; i<400; i++){
    ...
    Analys();                    
    TmpA = 2047-PavgA;    TmpB = 2047-PavgB;
    
    if(TmpA != 0) {
      KpA[KindA] += TmpA; StA = 0;           // CH_A 误差校正
    } else StA++; 
    
    if(TmpB != 0) {
      KpB[KindB] += TmpB; StB = 0;           // CH_B 误差校正
    } else StB++; 
...
void Zero_Align(void)
{
  GainA  = GainB  = x20;      
  KindA  = KindB  = HV;
  __Ctrl(AiRANGE, HV+AC+GND); __Ctrl(BiRANGE, HV+AC+GND); // 设置 LV 挡输入量程
  AiPosi(100);                BiPosi(100);                // 设置 LV 挡参考零点  
  Wait_mS(1000); 
  for(u8 i=0; i<100; i++) Analys(); 
  Align_Set();                      // 延时后校正零点误差 
  
  KindA  = KindB  = LV;
  __Ctrl(AiRANGE, LV+AC+GND); __Ctrl(BiRANGE, LV+AC+GND); // 设置 HV 挡输入量程
  AiPosi(100);                BiPosi(100);                // 设置 HV 挡参考零点 
  Wait_mS(1000); 
  Align_Set();                      // 延时后校正零点误差
}
/*******************************************************************************
...
//------------------ 数据预处理,生成近程指针提高数据调用效率 -----------------//
  ...
  s16  VtlA, ViA, VthA, VeA, VpA, VoA;            // VtA, 
  s16  VtlB, ViB, VthB, VeB, VpB, VoB;            // VtB, 
    ...
    for(n=0; n<300;){
      ...
      while(p > 0){
        ...
        if(Status == STOP){
          ...
          case MR1:  WaveLtdOut(BufC+n, FileBuff[8+n]     + Posi_3F1); break; 
          case MR2:  WaveLtdOut(BufC+n, FileBuff[8+300+n] + Posi_3F2); break; 
          case MR3:  WaveLtdOut(BufC+n, FileBuff[8+600+n] + Posi_3F3); break; 
          }
        }
        else{
...
void Auto_Fit(void)
{
  u8 i,j,k=0;
  s16 Tmp,Tmp1,Tmp2;
  s32 TmpA = (KgA[KindA]*4)/GK[GainA+5*KindA];
  s32 TmpB = (KgB[KindB]*4)/GK[GainB+5*KindB];
  
  if(Status != STOP){
    //======电压档自动选择==============
    for(i=0;i<9;i++){  
      //******A通道***********
      Tmp1 = ((((PmaxA-2048)*(TmpA))>>12)+100-PopMenu1_Value[CH1_Posi]) 
           - ((((PminA-2048)*(TmpA))>>12)+100-PopMenu1_Value[CH1_Posi]);
      ...
      //******B通道***********
      Tmp2 = ((((PmaxB-2048)*(TmpB))>>12)+100-PopMenu1_Value[CH2_Posi]) 
           - ((((PminB-2048)*(TmpB))>>12)+100-PopMenu1_Value[CH2_Posi]);
    ...
    if(!PopMenu1_Value[TRI_Ch]){//******A通道***********
      Tmp = (((((PmaxA-2048)*(TmpA))>>12)+100-PopMenu1_Value[CH1_Posi]) 
             - ((((PminA-2048)*(TmpA))>>12)+100-PopMenu1_Value[CH1_Posi]))/4;
      if((PopMenu1_Value[TRI_Ch1] > (((((PmidA-2048)*(TmpA))>>12)+100-PopMenu1_Value[CH1_Posi])  + Tmp))||
         (PopMenu1_Value[TRI_Ch1] < (((((PmidA-2048)*(TmpA))>>12)+100-PopMenu1_Value[CH1_Posi])  + Tmp))){
           if(fabs(((((PmidA-2048)*(TmpA))>>12)+100-PopMenu1_Value[CH1_Posi])+15)<195)           
             PopMenu1_Value[TRI_Ch1] = ((((PmidA-2048)*(TmpA))>>12)+100-PopMenu1_Value[CH1_Posi])+15 ;
           Process();
           Show_Title(); 
         }
      ...
      Tmp = (((((PmaxB-2048)*(TmpB))>>12)+100-PopMenu1_Value[CH2_Posi]) 
          - ((((PminB-2048)*(TmpB))>>12)+100-PopMenu1_Value[CH2_Posi]))/4;
      if((PopMenu1_Value[TRI_Ch2] > (((((PmidB-2048)*(TmpB))>>12)+100-PopMenu1_Value[CH2_Posi])  + Tmp))||
         (PopMenu1_Value[TRI_Ch2] < (((((PmidB-2048)*(TmpB))>>12)+100-PopMenu1_Value[CH2_Posi])  + Tmp))){
           if(fabs(((((PmidB-2048)*(TmpB))>>12)+100-PopMenu1_Value[CH2_Posi])+15)<195)
           PopMenu1_Value[TRI_Ch2] = ((((PmidB-2048)*(TmpB))>>12)+100-PopMenu1_Value[CH2_Posi])+15 ;
           Process();
           Show_Title(); 
         }
    ...
    if(((((PmidB-2048)*(TmpB))>>12)+100-PopMenu1_Value[CH2_Posi])>10){
      PopMenu1_Value[TRI_Ch]=1;
      Process();
      Update_Proc_All();
      Show_Title(); 
      ...
      for(i=0;i<9;i++){
    ...
    else if((((((PmaxB-2048)*(TmpB))>>12)+100-PopMenu1_Value[CH2_Posi]) 
         - ((((PminB-2048)*(TmpB))>>12)+100-PopMenu1_Value[CH2_Posi])) <10){
           //===========无信号时电压调2V==================
        ...
        if(((((PmidA-2048)*(TmpA))>>12)+100-PopMenu1_Value[CH1_Posi])>10){
      PopMenu1_Value[TRI_Ch]=0;
      Process();
      Update_Proc_All();
      Show_Title(); 
    ...
    else if((((((PmaxA-2048)*(TmpA))>>12)+100-PopMenu1_Value[CH1_Posi]) 
         - ((((PminA-2048)*(TmpA))>>12)+100-PopMenu1_Value[CH1_Posi])) <10){
      //===========无信号时电压调2V==================
    ...
    if(!PopMenu1_Value[TRI_Ch]){//******A通道***********
      ...
      for(j=0;j<11;j++){
        ...
        if((EdgeA > 180)&&(PopMenu1_Value[TIM_Base] > 1)){
      ...
      for(j=0;j<11;j++){
        ...
        if((EdgeB > 180)&&(PopMenu1_Value[TIM_Base] > 1)){
          PopMenu1_Value[TIM_Base]--;
          Set_Base(PopMenu1_Value[TIM_Base]);   // 硬件设置扫描时基档位
          //Process();
          Show_Title(); 
